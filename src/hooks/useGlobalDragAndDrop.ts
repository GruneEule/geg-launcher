        ...
        unlistenDragDrop = await currentWebviewWindow.onDragDropEvent(async (event: TauriEvent<unknown>) => {
          ...
          if (payload.type === 'hover') {
            ...
            const profilePackPath = droppedPaths.find(path => 
              path.toLowerCase().endsWith('.noriskpack') || path.toLowerCase().endsWith('.mrpack') || path.toLowerCase().endsWith('.gegpack') || path.toLowerCase().endsWith('.zip')
            );
            ...
            if (currentProfileId && currentContentType) {
              ...
              toast('Drop files onto an active profile content area to import them, or drop a .noriskpack/.gegpack/.mrpack/.zip file anywhere to import a profile.');
            }
