...
pub async fn import_profile_from_file(app_handle: tauri::AppHandle) -> Result<(), CommandError> {
    ...
    let dialog_result = tokio::task::spawn_blocking(move || {
        app_handle
            .dialog()
            .file()
            .add_filter("Modpack Files", &["mrpack", "noriskpack", "gegpack", "zip"])
            .set_title("Select Modpack File (.gegpack, .mrpack, .noriskpack or .zip)")
    ...
    if let Some(file_path_obj) = dialog_result {
        ...
        let new_profile_id = match file_extension.as_deref() {
            ...
            _ => {
                ...
                return Err(CommandError::from(AppError::Other(
                    "Invalid file type selected. Please select a .gegpack, .mrpack, .noriskpack, or .zip file."
                        .to_string(),
                )));
            }
        };
